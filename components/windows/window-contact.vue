        ...
        <div class="w-full h-full bg-slate-900 flex justify-center items-center flex-col">
            ...
            <suspense>
                ...
                <div class="p-2 flex gap-2 flex-col">
                    ...
                    <div class="flex-row flex items-center bg-gray-600 cursor-pointer group relative p-2 w-80 rounded-xl hover:rounded-none transition-all duration-100 ease-linear"
                         v-for="contact in contacts" :key="contact.name" @click="()=>{atBtnClick(contact)}">
                        ...
                        <span v-if="!store.isDeviceMobile"
                              class="sidebar-tooltip group-hover:scale-100">{{ contact.name }}</span>
                    </div>
