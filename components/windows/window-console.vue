...
function addText(texts) {
  if (!Array.isArray(texts)) texts = [texts]
    ...
    let text = texts.join("\n")
    let i = 0;
    screen.value += "\n"+ currentPath.value +"i>" + inputText.value + "\n\n"
...
<template>
  <Window title="Command Prompt" header-color="bg-gray-900" header-select-color="bg-black"  icon="ph:terminal-fill">
    <div class=" w-full h-full bg-slate-1000 " ref="thisWindow" @click="atConsoleClick">
      <div class="w-full h-full  text-gray-300 leading-[1.05] console overflow-y-scroll overflow-x-hidden" style="white-space: pre;">
        {{screen}}
        <br>{{currentPath}}>
        <input
            type="text"
