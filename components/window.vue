...
const winStyle = computed(() => {
  ...
  const cls = [
    ...
    !isSelected && "z-20",
    isDragging && "noSelect",
    "absolute w-[60rem] h-[34rem] windowStyle ",
  ].filter(Boolean).join(" ")
...
<template>
  <div :data-uuid="uuid" class="ring-1 ring-slate-500 flex flex-col transition-all duration-[50ms] ease-linear " ref="appWindow" :class="winStyle"
